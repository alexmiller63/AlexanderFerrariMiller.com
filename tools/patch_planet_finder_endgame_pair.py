...
old_sig = "    def viable_candidates(item, depth, *, consume_body_budget=True):\n"
new_sig = (
    "    def viable_candidates(item, depth, *, consume_body_budget=True, "
    "raw_probe_state=None, raw_probe_cap=None):\n"
)
if text.count(old_sig) != 1:
    raise SystemExit(f"expected one viable_candidates signature, found {text.count(old_sig)}")
...
if text.count(old_raw) != 1:
    raise SystemExit(f"expected one raw-position accounting block, found {text.count(old_raw)}")
...
print("Applied Planet Finder shared raw-probe endgame cap")
