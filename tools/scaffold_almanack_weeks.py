...
def week_nav(year: int, week: int) -> str:
    if week > 1:
        ...
        py = year - 1; prev = f'<a href="../../{py}/W{week_count(py):02d}/">← {py}-W{week_count(py):02d}</a>'
    if week < week_count(year):
        nxt = f'<a href="../W{week+1:02d}/">W{week+1:02d} →</a>'
    else:
        ny = year + 1; nxt = f'<a href="../../{ny}/W01/">{ny}-W01 →</a>'
...
def page(year: int, week: int, monday: dt.date) -> str:
    ...
    nav = week_nav(year, week)
    rows = calendar_rows(monday)
    body = f'''{nav}<h1>{title}</h1><p><strong>Week begins:</strong> {monday.strftime("Monday, %B")} {monday.day}, {monday.year}</p>
...
<h3>Sky Notes</h3><div class="sky-note"><p>Sky notes pending.</p></div>{nav}'''
