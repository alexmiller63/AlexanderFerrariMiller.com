...
def selected_weeks(sy,sw,ey,ew):
    validate(sy,sw); validate(ey,ew); d=dt.date.fromisocalendar(sy,sw,1); end=dt.date.fromisocalendar(ey,ew,1)
...
def year_nav(year,bottom=False):
    ...
    left=f'<a href="../../{previous}/{suffix}">← {previous}</a>' if previous else '<span class="nav-spacer" aria-hidden="true">—</span>'; center=f'<a href="../{suffix}">{year}</a>'; right=f'<a href="../../{following}/{suffix}">{following} →</a>' if following else '<span class="nav-spacer" aria-hidden="true">—</span>'
...
def week_nav(year,week,bottom=False): return '<nav class="weeknav week-position" aria-label="Week navigation">'+week_link(year,week,-7,bottom)+f'<span aria-current="page">ISO {year}-W{week:02d}</span>'+week_link(year,week,7,bottom)+'</nav>'
