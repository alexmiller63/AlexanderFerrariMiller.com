...
def inject(root,by_date):
 ...
 for page in pages_for_events(root,by_date):
  ...
  for day,labels in by_date.items():
   ...
   keep=[] if cell=="—" else [item for item in cell.split("<br>") if item and not is_messier_event(item)]
   keep.extend(labels); text,found=set_events(text,day,"<br>".join(keep) if keep else "—")
