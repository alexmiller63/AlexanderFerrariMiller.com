...
def validate(root: Path, events: dict[dt.date, list[str]]) -> None:
    """Validate final state: every expected observance is present exactly once."""
    ...
    for d, vals in events.items():
        ...
        for v in vals:
            ...
            if count != 1:
                raise SystemExit(
                    f"{root}: expected {v!r} exactly once on {d} in {page}, found {count}"
                )
...
def main() -> None:
    for year in YEARS:
        ...
        print(
            f"{year}: verified 25 asterism observances exactly once in each tree; "
            f"inserted source={i1}, public={i2}; updated {c1} source + {c2} public pages"
        )
