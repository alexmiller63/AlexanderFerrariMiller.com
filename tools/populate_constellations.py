...
def main():
 for year in requested_years():
  rows=build_rows(year); write_csv(year,rows); events=event_map(rows); p=inject(PUBLIC,events); validate(PUBLIC,events); identities={(r["abbr"],r.get("figure_part","")) for r in rows}; print(f"{year}: {len(rows)} constellation-center ISO-year occurrence(s) across {len(identities)} represented identities; updated {p} public pages")
