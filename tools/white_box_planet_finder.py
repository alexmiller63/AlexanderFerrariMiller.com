...
from planet_finder_geometry import (
    BODY_SYMBOLS, CANONICAL, FinderMode, RI, candidate_positions,
...
def candidate_generation_audit(mode, bodies, scale=2.0):
    """Trace raw proposals through immutable filtering, before DFS exists."""
    reserved=reserved_boxes(mode)
    print("WHITE BOX CANDIDATE GENERATION AUDIT", flush=True)
    for _,name,lon in bodies:
        w,h=label_size(mode,name)
        raw=list(candidate_positions(lon,scale))
        diag={}
        legal=list(legal_candidate_positions(lon,w,h,reserved,scale,diagnostic=diag))
        audits=diag.get("immutable_candidate_audit",[])
        rejected_reserved=sum(1 for a in audits if a[2]=="reserved")
        rejected_rim=sum(1 for a in audits if a[2]=="rim")
        reasons={}
        for _,_,reason,detail in audits:
            key=reason if reason!="reserved" else f"reserved:{detail}"
        ...
        if not legal:
            ...
            for i,(x,y,reason,detail) in enumerate(audits[:20],1):
                print(f"    reject#{i} center=({x:.1f},{y:.1f}) reason={reason} detail={detail}",flush=True)
...
def first_level_forward_audit(mode,bodies,displacement_scale=2.0,first_limit=12):
    ...
    for _,first_name,first_lon in bodies:
        fw,fh=label_size(mode,first_name); anchor=xy(first_lon,RI-5); tested=0
        ...
        for _,_,first_box in legal_candidate_positions(first_lon,fw,fh,reserved,displacement_scale):
            ...
            for _,future_name,future_lon in bodies:
                ...
                w,h=label_size(mode,future_name); a=xy(future_lon,RI-5)
                c={"raw":0,"overlap":0,"route":0,"rim":0,"leader":0}; witness=False
            ...
            if dead is None:
                print(f"  first-candidate={tested}: SURVIVES center=({first_box.x:.1f},{first_box.y:.1f})",flush=True); break
...
def main():
    ...
    candidate_generation_audit(mode,bodies)
    ...
    try: result=layout(mode,bodies,target_solutions=a.candidates,budget=budget,context_label="WHITE-BOX-CROWDED")
        ...
        print(f"WHITE BOX RESULT: FAILURE after {time.monotonic()-started:.3f}s: {type(exc).__name__}: {exc}"); raise
