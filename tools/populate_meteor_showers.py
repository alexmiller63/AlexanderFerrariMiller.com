...
EPHEMERIS-PROVENANCE.md; the reference working list is the
...
def crossing(samples, target_deg, year):
    ...
    for k in range(
        int((values[0] - target_deg) // 360) - 1,
        int((values[-1] - target_deg) // 360) + 2,
    ):
        target = target_deg + 360 * k
        for i in range(len(values) - 1):
            if values[i] <= target <= values[i + 1]:
                ts = interpolate_time(
                    times[i], values[i], times[i + 1], values[i + 1], target
                )
                if ts.publication_date().year == year:
                    return ts
                break
    raise RuntimeError(f"No {year} solar-longitude crossing for {target_deg}°")


def longitude_at(samples, ts: AstroInstant):
    for i in range(len(samples) - 1):
        t0, lon0 = samples[i]
        t1, lon1 = samples[i + 1]
        if t0 <= ts <= t1:
            delta = ((lon1 - lon0 + 180) % 360) - 180
            span = t1.jd_tdb - t0.jd_tdb
            fraction = 0.0 if span == 0.0 else (ts.jd_tdb - t0.jd_tdb) / span
            return (lon0 + delta * fraction) % 360
    raise RuntimeError(
        f"Timestamp JDTDB {ts.jd_tdb:.9f} outside ephemeris sample range"
    )


def moon_context(sun, moon, ts):
    elongation = abs(
        ((longitude_at(moon, ts) - longitude_at(sun, ts) + 180) % 360) - 180
    )
    illumination = (1 - math.cos(math.radians(elongation))) / 2
    percent = int(round(illumination * 100))
    interference = (
        "little moonlight interference"
        if illumination < .20
        else "moderate moonlight interference"
        if illumination < .50
        else "substantial moonlight interference"
        if illumination < .80
        else "strong moonlight interference"
    )
...
def observing_guidance(p):
    base = (
        "Use a dark site, allow 20–30 minutes for dark adaptation, and observe "
        "with the unaided eye."
    )
    return base + (
        " Put the Moon behind a building, tree, or other obstruction and watch "
        "the darkest open sky."
        if p >= 50
        else " Watch a broad area of sky rather than staring directly at the radiant."
    )
...
def shower_events(year):
    ...
    for name, lon, constellation, zhr in SHOWERS:
        ...
        mp, interference = moon_context(sun, moon, ts)
        events.append((name, lon, constellation, zhr, ts, mp, interference))
    return events
...
def calendar_label(e):
    ...
    return (
        f"{METEOR_GLYPH} {name} peak · radiant in {constellation} · "
        f"expected ZHR ≈ {zhr} · Moon {mp}% illuminated: {interference} · "
        f"{observing_guidance(mp)}"
    )
...
def patch_page(path, additions):
    ...
    for d in page_dates(path):
        ...
        keep = [
            x for x in existing
            if not any(name in x and "peak" in x for name in SHOWER_NAMES)
        ]
        merged = additions.get(d, []) + keep
        text, found = set_events(text, d, "<br>".join(merged) if merged else "—")
        if not found:
            raise RuntimeError(f"Could not update {d} in {path}")
    if text != original:
...
def populate_year(year):
    ...
    additions = {}
    for e in maxima:
        additions.setdefault(e[4].publication_date(), []).append(calendar_label(e))
        ...
        "showers": [
            {
                ...
                "radiant_constellation": c,
                "expected_zhr": z,
                "jd_tdb": ts.jd_tdb,
                "utc": ts.publication_utc().isoformat().replace("+00:00", "Z"),
                "moon_illumination_percent": mp,
                "moon_interference": i,
                "observing_guidance": observing_guidance(mp),
                "calendar_label": calendar_label((n, lon, c, z, ts, mp, i)),
            }
            for n, lon, c, z, ts, mp, i in maxima
        ],
    }
...
def parse_years():
    ...
    p.add_argument("years", metavar="YEAR", type=int, nargs="+")
    a = p.parse_args()
    years = list(dict.fromkeys(a.years))
    for y in years:
        if not 1900 <= y <= 2100:
            p.error(f"YEAR must be between 1900 and 2100: {y}")
    return years


def main():
    years = parse_years()
    total = sum(populate_year(y) for y in years)
