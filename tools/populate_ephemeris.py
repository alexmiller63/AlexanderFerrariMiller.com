...
def update_year(year):
 ...
 # Accept both canonical tables and preserved legacy tables that predate the
 # ephemeris class. Every successful match is rewritten to canonical markup.
 table=r'<table(?:\s+class="ephemeris")?>.*?</table>'
 pat=re.compile(r'<h3>Weekly Solar-System Ephemeris</h3>\s*<p><strong>Snapshot:</strong>.*?</p>\s*(?:<p><strong>Naked Eye</strong></p>\s*)?'+table+r'\s*<p><strong>Extended targets:</strong></p>\s*'+table+r'\s*(?:<p class="ephemeris-note">.*?</p>)?',re.DOTALL); changed=0
