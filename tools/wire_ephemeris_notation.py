...
(function(){
  ...
  function aidKind(img){const src=(img.getAttribute('src')||'').split('/').pop()||'';return src.replace(/\.svg(?:\?.*)?$/,'');}
  function prepareObservingAids(){
    document.querySelectorAll('table.calendar .visibility-magnitude, table.ephemeris tr.ephemeris-visibility td').forEach(function(container){
      ...
      const kinds=glyphs.map(aidKind);let label;
      if(kinds.length===2&&kinds.every(function(kind){return kind==='telescope';}))label='Substantial telescope';
      else label=kinds.map(function(kind){return observingNames[kind]||'';}).filter(Boolean).join(' / ');
      if(!label)return;
      const greekHtml=glyphs.map(function(img){return img.outerHTML;}).join('');
      const span=document.createElement('span');span.className='observing-notation-item';span.dataset.greekHtml=greekHtml;span.dataset.latin=label;span.dataset.mixedHtml=greekHtml+' '+label;glyphs[0].before(span);glyphs.forEach(function(img){img.remove();});
    });
  }
    ...
    document.querySelectorAll('table.ephemeris').forEach(function(table){
      table.querySelectorAll('th').forEach(function(th){
        ...
        for(const [name,glyph] of Object.entries(bodies)){
          if(txt===glyph||txt===name||txt===glyph+' '+name){th.textContent=glyph+VS;item(th,glyph+VS,name,glyph+VS+' '+name,'ephemeris-notation-item');break;}
        }
      });
      table.querySelectorAll('td .ephemeris-symbol').forEach(function(el){if(el.dataset.greek)return;const glyph=el.textContent.replace(/\ufe0e/g,'').trim().charAt(0);if(signs[glyph]) item(el,glyph+VS,signs[glyph],glyph+VS+' '+signs[glyph],'ephemeris-notation-item');});
    });
  }
  function setObservingMode(mode){document.querySelectorAll('.observing-notation-item').forEach(function(el){if(mode==='latin')el.textContent=el.dataset.latin||'';else if(mode==='mixed')el.innerHTML=el.dataset.mixedHtml||el.dataset.greekHtml||'';else el.innerHTML=el.dataset.greekHtml||'';});}
  function setFinder(mode){document.querySelectorAll('[data-finder-image]').forEach(function(f){f.style.display=f.dataset.finderImage===mode?'block':'none';});document.querySelectorAll('.w15-finder-strip [data-finder-mode]').forEach(function(f){const active=f.dataset.finderMode===mode;f.classList.toggle('is-active',active);f.hidden=!active;f.style.display=active?'block':'none';});}
  ...
  function setModeKeepingControlStill(button,mode){const control=button.closest('.section-notation-toggle')||button;const before=control.getBoundingClientRect().top;setMode(mode);const after=control.getBoundingClientRect().top;const delta=after-before;if(Math.abs(delta)>0.5) window.scrollBy(0,delta);}
  document.querySelectorAll('[data-bayer-mode]').forEach(function(button){button.addEventListener('click',function(){setModeKeepingControlStill(button,button.dataset.bayerMode);});});
  let initial='greek';try{const s=localStorage.getItem('star-almanack-bayer-mode');if(s==='greek'||s==='latin'||s==='mixed')initial=s}catch(_){}
  /* Always normalize generated markup from its canonical Greek/symbol state before
     restoring a saved Latin or Mixed preference. This makes first-load behavior
     identical to an explicit mode switch and prevents duplicated body names. */
  prepareCalendar();prepareObservingAids();prepareEphemeris();
  document.querySelectorAll('.calendar-notation-item,.ephemeris-notation-item').forEach(function(el){renderNotation(el,el.dataset.greek,'greek');});
  setObservingMode('greek');
  setMode(initial);
})();
</script>'''
...
def requested_pages() -> tuple[Path, ...]:
    ...
    for root in sorted(Path("almanack").iterdir()):
        ...
        for page in sorted(root.glob("W[0-9][0-9]/index.html")):
            ...
            try:monday = __import__("datetime").date.fromisocalendar(year, week, 1)
            except ValueError:continue
            if start <= monday <= end:pages.append(page)
    return tuple(pages)
...
for page in pages:
    ...
    if html2 != html:
        page.write_text(html2, encoding='utf-8');changed += 1
