...
ROOT=Path(__file__).resolve().parents[1]; SRC=ROOT/"Historical"; PUBLIC=ROOT/"almanack"; SOURCE_SITE=SRC/"site"
...
def calendar_label(record,finest_ids,asterism_ids):
 cid=record["caldwell"].strip(); catalog=record.get("catalog","").strip(); name=record.get("name","").strip(); object_type=TYPE_LABELS.get(record.get("type","").strip(),"deep-sky object"); constellation=CONSTELLATIONS.get(record.get("con","").strip(),record.get("con","").strip()); identity=[cid]
...
def events_for(rows,finest_ids,asterism_ids):
 ...
 for r in rows:events[dt.date.fromisoformat(r["best_date"])].append(calendar_label(r,finest_ids,asterism_ids))
 return events
...
def inject(root,events):
 ...
 for page in pages_for_events(root,events):
  ...
  for day,labels in events.items():
   ...
   keep=[] if cell=="—" else [i for i in cell.split("<br>") if i]
   for label in labels:
