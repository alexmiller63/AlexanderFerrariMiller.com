...
def extract_first_image(md_text: str) -> tuple[dict[str, str] | None, str]:
    """
    Extract the first Markdown image from the resume.

    Supports:
        ![Alt text](path)
        ![Alt text](path){width=2in}

    Returns:
        (image_info, markdown_without_first_image)
...
def build_html(
    ...
    if image:
        ...
        header_class = "resume-header resume-header-with-image"
    else:
        portrait_html = ""
...
def escape_table_text(value: str) -> str:
    """
    Escape characters that can break a Pandoc Markdown table.
    """

    return value.replace("|", "\\|")


def build_pandoc_markdown(
    header: dict[str, str],
    md_text: str,
    image: dict[str, str] | None,
) -> str:
    """
    Build the document used for DOCX and ODT.

    When a portrait exists, use a two-column Markdown table so Word,
    LibreOffice, and ODT keep the contact header on the left and image
    on the right.
    ...
    name = escape_table_text(header["name"])
    street = escape_table_text(header["street"])
    city = escape_table_text(header["city"])
    phone = escape_table_text(header["phone"])
    email = escape_table_text(header["email"])
    website = escape_table_text(header["website"])

    alt = image["alt"] or f"{header['name']} portrait"
    ...
    image_cell = f"![{alt}]({image_path}){attributes}"

    header_table = (
        "| Contact | Portrait |\n"
        "|:--|--:|\n"
        f"| {contact_cell} | {image_cell} |\n"
    )

    return f"{header_table}\n{md_text.strip()}\n"
...
def generate_pdf_from_docx(
    ...
    if not soffice:
        raise SystemExit(
            ...
            "Install LibreOffice and make sure libreoffice "
            "or soffice is on PATH."
        )
...
def generate(
    ...
    image, body_md = extract_first_image(
        original_md
    )
