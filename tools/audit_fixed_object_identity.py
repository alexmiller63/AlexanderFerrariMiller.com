...
Read-only with respect to canonical source data. Emits candidate records,
provisional exact-identifier reconciliation groups, and contradiction checks.
No fixed_object_id values are assigned and coordinate-only matches are not merged.
"""
from __future__ import annotations
import csv, json, math, re
from collections import Counter, defaultdict
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; SRC=ROOT/'Star-Almanack-Repo'; OUT=SRC/'generated'/'fixed-object-identity-audit.json'
...
def parse_fixed_simple_yaml(p):
    ss=schemas(); result={k:[] for k in ss}; current=None; in_schema=True
    for raw in p.read_text(encoding='utf-8').splitlines():
        if raw and not raw.startswith(' ') and raw.rstrip().endswith(':'):
            key=raw.strip()[:-1]; in_schema=key=='schema'; current=None if in_schema else(key if key in ss else None); continue
        if in_schema or current is None:continue
        s=raw.strip()
...
def reconcile(cs):
    ...
    for n,ids in enumerate(sorted(grouped.values(),key=min),1):
        ...
        if len(ids)>1:
            ...
            if len(constellations)>1:contradictions.append({'kind':'constellation','values':constellations})
            types=sorted({r['object_type'] for r in rows if r['object_type']})
            # Different source taxonomies can use different type codes; report rather than auto-reject.
            ...
            for x in range(len(rows)):
                for y in range(x+1,len(rows)):
                    ...
                    if sep is not None:max_sep=max(max_sep,sep)
...
def main():
    cs=[]; warnings=[]; fixed=parse_fixed_simple_yaml(SRC/'fixed-objects.yaml')
    for section,rows in fixed.items():
        for n,vals in enumerate(rows,1):
            r=row_dict(section,vals); ids=[]; key=f'{section}:{n}'
            if section=='messier':
                ids.append(('messier',r['id'])); ident=catalog_identifier(r['ngc']); ids+=([ident] if ident else []); key=str(r['id'])
            elif section=='bayer':ids.append(('bayer',f"{r['bayer']} {r['con']}")); key=f"{r['bayer']} {r['con']}"
            elif section=='special':
                ids.append(('special',r['id'])); ident=catalog_identifier(r['catalog']); ids+=([ident] if ident else []); key=str(r['id'])
    ...
    groups,by=reconcile(cs); overlaps=[{'namespace':ns,'value':v,'candidate_ids':ids} for (ns,v),ids in sorted(by.items()) if len(ids)>1]
    ...
    counts=Counter(c['source'] for c in cs); icounts=Counter(i['namespace'] for c in cs for i in c['identifiers']); no_cross=[]; no_any=[]
    for c in cs:
        if not c['identifiers']:no_any.append(c['candidate_id'])
        if not any(i['namespace'] in CROSS_SOURCE_NAMESPACES for i in c['identifiers']):no_cross.append(c['candidate_id'])
    merged=[g for g in groups if g['candidate_count']>1]; review=[g for g in merged if g['contradictions']]; validated=[g for g in merged if not g['contradictions']]
    result={'schema_version':4,'purpose':'pre-migration physical fixed-object identity audit; no permanent IDs assigned','canonical_source':'Star-Almanack-Repo/fixed-objects.yaml','candidate_count':len(cs),'provisional_reconciliation_group_count':len(groups),'provisional_merged_group_count':len(merged),'provisional_singleton_group_count':len(groups)-len(merged),'validated_exact_identifier_group_count':len(validated),'contradiction_review_group_count':len(review),'source_counts':dict(sorted(counts.items())),'identifier_counts':dict(sorted(icounts.items())),'cross_source_identifier_namespaces':sorted(CROSS_SOURCE_NAMESPACES),'stable_source_identifier_namespaces':sorted(STABLE_SOURCE_NAMESPACES),'cross_source_identifier_overlap_count':len(overlaps),'cross_source_identifier_overlaps':overlaps,'candidates_without_cross_source_identifier':no_cross,'candidates_without_any_identifier':no_any,'explicit_overlap_counts':{k:len(v) for k,v in explicit.items()},'warnings':warnings,'provisional_reconciliation_groups':groups,'candidates':cs,'explicit_overlap_records':explicit}
    OUT.parent.mkdir(parents=True,exist_ok=True); OUT.write_text(json.dumps(result,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    print(f'Wrote {OUT.relative_to(ROOT)}'); print(f'Candidates: {len(cs)}'); print(f'Provisional reconciliation groups: {len(groups)}'); print(f'Provisional merged groups: {len(merged)}'); print(f'Validated exact-ID groups: {len(validated)}'); print(f'Groups requiring contradiction review: {len(review)}'); print(f'Without cross-source ID: {len(no_cross)}'); print(f'Without any identifier: {len(no_any)}'); print('No fixed_object_id values were assigned.')
