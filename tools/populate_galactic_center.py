...
engine used by ``populate_calendar.py``.  The Sgr A* ICRS/J2000 position is
precessed to the date before the longitude comparison is made.
"""
...
def julian_date(ts: datetime) -> float:
    return 2440587.5 + ts.timestamp() / 86400.0


def galactic_center_ecliptic_longitude(ts: datetime) -> float:
    ...
    """
    t = (julian_date(ts) - 2451545.0) / 36525.0
...
def conjunctions(sun_samples: list[tuple[datetime, float]]) -> list[datetime]:
    """Find negative-to-positive Sun−GC longitude crossings."""
    out: list[datetime] = []
    for (t0, sun0), (t1, sun1) in zip(sun_samples, sun_samples[1:]):
        d0 = signed_angle(sun0 - galactic_center_ecliptic_longitude(t0))
        d1 = signed_angle(sun1 - galactic_center_ecliptic_longitude(t1))
        # The opposition wrap is nearly 360 degrees; reject it explicitly.
        if d0 <= 0.0 < d1 and 0.0 < d1 - d0 < 10.0:
            fraction = -d0 / (d1 - d0)
            out.append(t0 + (t1 - t0) * fraction)
    return out
...
def patch_page(path: Path, event_times: dict[date, datetime]) -> bool:
    if not path.exists():
        return False
...
def expected_for_year(year: int) -> dict[date, datetime]:
    ...
    hits = [ts for ts in conjunctions(sun) if first <= ts.date() <= last]
    if len(hits) != 1:
        raise RuntimeError(f"Expected exactly one Galactic Center conjunction in ISO year {year}; found {len(hits)}")
    return {hits[0].date(): hits[0]}
...
def count_occurrences(base: Path, year: int) -> int:
    ...
    for week in range(1, weeks + 1):
        page = base / str(year) / f"W{week:02d}" / "index.html"
        if not page.exists():
            continue
...
def populate_year(year: int) -> int:
    ...
    for base in (SOURCE_ROOT, PUBLIC_ROOT):
        ...
        if count != 1:
            raise RuntimeError(f"{base}: expected exactly one Galactic Center event for {year}, found {count}")
    ts = next(iter(events.values()))
    print(f"{year}: Galactic Center conjunction {ts.isoformat().replace('+00:00', 'Z')}; {changed} pages updated")
    return changed
...
def parse_args() -> tuple[list[int], bool]:
    ...
    parser.add_argument("--verify-only", action="store_true", help="Verify exactly one event per requested year without editing")
...
def main() -> None:
    ...
    if verify_only:
        ...
        print(f"Updated {total} calendar page files for Galactic Center: {' '.join(map(str, years))}")
