...
The first machine-readable relationship layer is now stored at `database/catalog-entry-targets.json`. It records these eight exception cases without assigning permanent `fixed_object_id` values. During migration it may use source references, external object identifiers, and audit candidate IDs as temporary references; those temporary references must be replaced or resolved when the permanent physical-object registry is created.
